...
const processPosts = (posts) => {
	...
	return Promise.map(posts, post => savePostIntoDbIfNew(post).thenReturn(post).then(incrPostCrawlCount), { concurrency: 5 })
	.thenReturn(posts)
...
const grabAndSavePostScore = (posts) => Promise.map(posts, post => grabPostCounts(post.post_id).then(savePostScores), {concurrency: 4})
	.catch(err => console.log(err, err.stack))
